...
public class Transaction {
   ...
   private static final int CKPT_PERIOD = 5; // for hw 4
   private static List<Integer> txs = new ArrayList<>(); // for hw 4
   ...
   public Transaction(FileMgr fm, LogMgr lm, BufferMgr bm) {
      ...
      checkpointIfNeeded(txnum);   // added for hw4
      txs.add(new Integer(txnum)); // added for hw4
      System.out.println("new transaction: " + nextTxNum);
   }
   
   // New method for hw4. It is synchronized so that 
   // new txs must wait until the checkpoint is finished.
   ...
   public void commit() {
      ...
      concurMgr.release();
      mybuffers.unpinAll();
      txs.remove(new Integer(txnum)); // added for hw4
   }
